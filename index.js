...
let ADMIN_QUARANTINE_CHANNEL_ID = process.env.ADMIN_QUARANTINE_CHANNEL_ID || '1410019894568681617'; // Owner Commands channel
...
async function sendLogMessage(guild, embed, components = []) {
    try {
        // Use the Owner Logs channel ID (1410019894568681617)
        const ownerLogsChannelId = '1410019894568681617';
        const logsChannel = guild.channels.cache.get(ownerLogsChannelId);
        ...
        if (logsChannel) {
            ...
            console.log(`✅ Log message sent to Owner Logs channel: ${logsChannel.name}`);
        } else {
            console.error(`❌ Owner Logs channel not found: ${ownerLogsChannelId}`);
            // Try fallback to original LOGS_CHANNEL_ID
            const fallbackChannel = guild.channels.cache.get(LOGS_CHANNEL_ID);
            if (fallbackChannel) {
        ...
        setTimeout(async () => {
            if (quarantinedUsers.has(userId)) {
                try {
                    ...
                    await sendLogMessage(guild, releaseEmbed);
            ...
            try {
                ...
                if (inviterMember) {
                    ...
                    if (quarantineSuccess) {
                        ...
                        // Send warning DM to inviter
                        try {
                            const warningEmbed = new EmbedBuilder()
                                .setColor('#FF0000')
                                .setTitle('🚨 SECURITY VIOLATION - Bot Addition Blocked')
                                .setDescription(`Your attempt to add an unauthorized bot to **${guild.name}** has been blocked and you have been quarantined.`)
                                .addFields(
                                    { name: '🤖 Blocked Bot', value: `${bot.user.username} (\`${bot.user.id}\`)`, inline: true },
                                    { name: '⏰ Quarantine Duration', value: '1 hour', inline: true },
                                    { name: '🛡️ Reason', value: 'Bot not in server whitelist', inline: false },
                                    { name: '📋 Server Policy', value: 'Only pre-approved bots are allowed. Contact server administrators for bot approval.', inline: false },
                                    { name: '⚠️ Warning', value: 'Repeated violations may result in permanent removal from the server.', inline: false }
                                )
...
// Ultra-fast response system with <1ms response time
...
// Monitor for mass changes that might bypass audit logs
client.on('channelDelete', async channel => {
    ...
    try {
        ...
        // Fetch recent audit logs to find who deleted the channel
        ...
        if (deleteLog && deleteLog.executor && !isProtectionImmune(deleteLog.executor.id, guild.id)) {
            ...
            const recentChannelDeletes = userActionTracking.get(deleteLog.executor.id)?.actions.filter(
                a => a.type.includes('CHANNEL_DELETE') && Date.now() - a.timestamp < 60000
            ) || [];

            if (recentChannelDeletes.length >= 3) {
                await handleProtectionViolation(guild, deleteLog.executor, 'MASS_CHANNEL_DELETE',
                    `CRITICAL: Deleted ${recentChannelDeletes.length + 1} channels in 1 minute - POSSIBLE NUKE ATTEMPT`);
            }
        }
    } catch (error) {
        console.error('Error monitoring channel deletion:', error);
    }
});
...
// Handle new member joins (including bot detection and quarantine evasion)
client.on('guildMemberAdd', async member => {
    const guild = member.guild;

    try {
        // Ultra-fast response for new member joins
        ultraFastResponse(async () => {
            // Check for quarantine evasion first (human users)
            if (!member.user.bot) {
                const evasionDetected = await handleQuarantineEvasion(member);
                if (evasionDetected) {
                    return; // Stop processing if quarantine evasion was handled
                }
            }
        });
        // Check if the new member is a bot
        if (member.user.bot) {
            console.log(`🔍 Bot detected joining server: ${member.user.username} (${member.user.id})`);

            // Check if bot is in whitelist
            if (!WHITELISTED_BOTS.has(member.user.id)) {
                console.log(`❌ Bot ${member.user.username} is NOT whitelisted!`);

                // Check if bot was previously flagged
                if (flaggedBots.has(member.user.id)) {
                    console.log(`🚨 Previously flagged bot attempting re-entry: ${member.user.username}`);

                    // Immediate ban for flagged bots
                    try {
                        await guild.bans.create(member.user.id, {
                            reason: 'FLAGGED BOT - Previously blocked. Auto-banned on re-entry attempt.',
                            deleteMessageSeconds: 7 * 24 * 60 * 60
                        });
                        console.log(`✅ Immediately banned flagged bot: ${member.user.username}`);

                        const flaggedBotEmbed = new EmbedBuilder()
                            .setColor('#8B0000')
                            .setTitle('🛡️ FLAGGED BOT BLOCKED')
                            .setDescription(`**SECURITY:** Previously flagged bot attempted re-entry and was immediately banned.`)
                            .addFields(
                                { name: '🤖 Bot', value: `${member.user.username} (\`${member.user.id}\`)`, inline: true },
                                { name: '🚨 Status', value: 'Flagged bot - immediate ban', inline: true },
                                { name: '🛡️ Action', value: 'Auto-banned on detection', inline: true }
                            )
                            .setTimestamp();

                        await sendLogMessage(guild, flaggedBotEmbed);
                    } catch (error) {
                        console.error('Error banning flagged bot:', error);
                    }
                    return;
                }
                ...
                try {
                    ...
                    if (inviteLog) {
                        ...
                        console.log(`📋 Bot inviter identified: ${inviter.username} (${inviter.id})`);
                    }
                } catch (auditError) {
                    console.log('Could not fetch audit logs for bot invite:', auditError.message);
                }

                // Handle unauthorized bot
                await handleUnauthorizedBot(guild, member, inviter);

            } else {
                // Bot is whitelisted - log successful entry
                console.log(`✅ Whitelisted bot joined: ${member.user.username}`);

                const whitelistedBotEmbed = new EmbedBuilder()
                    .setColor('#00FF00')
                    .setTitle('✅ Authorized Bot Joined')
                    .setDescription(`**APPROVED:** Whitelisted bot successfully joined the server.`)
                    .addFields(
                        { name: '🤖 Bot', value: `${member.user.username} (\`${member.user.id}\`)`, inline: true },
                        { name: '✅ Status', value: 'Whitelisted - approved', inline: true },
                        { name: '🛡️ Security', value: 'Passed all checks', inline: true }
                    )
                    .setThumbnail(member.user.displayAvatarURL({ dynamic: true, size: 64 }))
                    .setTimestamp();

                await sendLogMessage(guild, whitelistedBotEmbed);
            }
        } else {
            // Regular human member joined
...
client.on('messageCreate', async message => {
    ...
    const commandLikePatterns = [
        ...
        'wbtestan', 'set', 'setinterimrole', 'recovery', 'nightmode',
        // Voice Management Commands
        'vmute', 'vunmute', 'vmuteall', 'vunmuteall', 'vdefend', 'vundefend', 'vdefendall', 'vundefendall', 'vdefended',
        // New Voice Commands
        'muv', 'muvu', 'hvcm',
